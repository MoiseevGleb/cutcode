...
@section('content')
    <x-forms.auth-forms action="{{ route('store') }}" method="POST">
        @csrf
        ...
        <x-slot:socialAuth>
            <ul class="space-y-3 my-2">
                <li>
                    <a href="{{ route('socialite.redirect', 'github') }}" class="relative flex justify-center items-center h-14 px-12 rounded-lg border border-[#A07BF0] bg-white/20 hover:bg-white/20 active:bg-white/10 active:translate-y-0.5">
        ...
        <x-slot:buttons>
            <div class="space-y-3 mt-5">
                <div class="text-xxs md:text-xs"><a href="{{ route('login') }}" class="text-white hover:text-white/70 font-bold">Уже есть аккаунт? Войти.</a></div>
            </div>
        </x-slot:buttons>
